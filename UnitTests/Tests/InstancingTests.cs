...
namespace UnitTests.Tests
{
    ...
    public class InstancingTests
    {
        ...
        [Test]
        public void NewProgram_WithCustomTerminalSystemFake_RunsE2E()
        {
            // Arrange
            var terminalSystem = A.Fake<IMyGridTerminalSystem>();
            var fakeConnector = A.Fake<IMyShipConnector>();
            A.CallTo(terminalSystem)
                .Where(call => call.Method.Name == "GetBlocksOfType")
                .WithVoidReturnType()
                .Invokes(
                    (List<IMyShipConnector> connectors, Func<IMyShipConnector, bool> collect) =>
                    {
                        connectors.Add(fakeConnector);
                    }
                );

            // Act
            var program = Gateway.CreateProgram<Program>()
                .WithGridTerminalSystem(terminalSystem)
                .Build();
            program.Main("", UpdateType.None);
            
            // Assert
            A.CallToSet(() => fakeConnector.Enabled).MustHaveHappened();
        }
    }
}